...
export function OffencesCrimesCountPieChart({ title, description, dateRange }: CustomPieChartProps) {
  const { data: offencesCrimesCount, isLoading, error } = useQuery({
    queryKey: ["offencesCrimesCount", dateRange],
    queryFn: () => fetchOffencesCrimesCount({
      rangeDate: dateRange
  ...
  const data = offencesCrimesCount?.offencesStats.map((item, index) => ({
  ...
  console.log(offencesCrimesCount.offencesStats);
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1 pb-0">
        <ChartContainer config={config} className="mx-auto aspect-square max-h-[250px]">
          <PieChart>
            <ChartTooltip content={<CustomTooltip active={true} payload={true} />} />
